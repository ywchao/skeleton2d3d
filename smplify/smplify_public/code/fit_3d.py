...
# --------------------Camera estimation --------------------
def guess_init(model, focal_length, j2d, init_pose, dataset):
    ...
    if dataset == 'h36m':
        smpl_ids = [8, 5, 2, 1, 4, 7, 21, 19, 17, 16, 18, 20, 15]
...
def initialize_camera(model,
    ...
    init_t = guess_init(model, flength, j2d, init_pose, dataset)
    t = ch.array(init_t)
...
def main(base_dir,
    ...
    if dataset == 'h36m':
        img_dir = join(abspath(base_dir), '../../data/h36m/frames')
        anno_file = join(abspath(base_dir), '../../data/h36m/val.mat')
        anno = sio.loadmat(anno_file)
        ind2sub = anno['ind2sub']
    assert 'img_dir' in locals(), 'unsupported dataset'
    ...
    if not use_neutral:
        ...
        if dataset == 'lsp':
            ...
            with open(join(data_dir, 'lsp_gender.csv')) as f:
                genders = f.readlines()
        if dataset == 'h36m':
            # Use ground-truth gender in H36M
            genders = [s == 5 for s in ind2sub[:,0]]
    ...
    if dataset == 'h36m':
        ...
        pred_dir = join(abspath(base_dir), '../../exp/h36m/{}/eval_val'.format(exp_name))
        hmap_dir = join(abspath(base_dir), '../../exp/h36m/{}/hmap_val'.format(exp_name))
        est = np.zeros([3, 13, ind2sub.shape[0]], dtype='float64')
        for ind in xrange(ind2sub.shape[0]):
            pred_file = '{}/{:05}.mat'.format(pred_dir, ind + 1)
            pred = sio.loadmat(pred_file)
            est[0:2, :, ind] = pred['eval'].T
            # est[2, :, ind] = 1.  # Set confidence to 1.0 for now
            hmap_file = '{}/{:05}.mat'.format(hmap_dir, ind + 1)
            hmap = sio.loadmat(hmap_file)
            est[2, :, ind] = np.amax(hmap['hmap'], axis=(1, 2))
        # Reorder joints to be consistent with lsp
        joints = [11, 9, 7, 8, 10, 12, 5, 3, 1, 2, 4, 6, 0];
        est = est[:, joints, :]
    ...
    if dataset == 'h36m':
        ...
        for ind in xrange(ind2sub.shape[0]):
            cam = ind % 4 + 1
            name = '{:02d}/{:02d}/{:1d}/{:1d}_{:04d}.jpg'.format(
                ind2sub[ind][0], ind2sub[ind][1], ind2sub[ind][2], cam, ind2sub[ind][3])
            path = join(abspath(base_dir), '../../data/h36m/frames/{}'.format(name))
            img_paths.append(path)
        # # Unable to converge: 6764, 9211
        # # Divided by zero: 9009
        # skip_ind = [6764, 9009, 9211]
        # Unable to converge: 4667
        # Divided by zero: 9009
        skip_ind = [4667, 9009]
    # Get the start and end indices of the batch
    length = len(img_paths)
    interval = int(np.ceil(float(length) / n_batches))
    ss = np.arange(0, length - 1, interval)
    ss = np.append(ss, length)
    sid = ss[batch_id - 1]
    eid = ss[batch_id]
    # Start running
    for ind, img_path in enumerate(img_paths):
        if ind < sid or ind >= eid:
            continue
        if ind in skip_ind:
            continue
        if dataset == 'lsp':
        ...
        if dataset == 'h36m':
            out_path = '%s/%05d.mat' % (out_dir, ind + 1)
        if not exists(out_path):
            ...
            if dataset == 'h36m':
                # Retrieve joint coordinates
                (_, A_global) = global_rigid_transformation(
                    params['pose'], model.J, model.kintree_table, xp=ch)
                Jtr = ch.vstack([g[:3, 3] for g in A_global])
                params['Jtr'] = np.array(Jtr)
                sio.savemat(out_path, params)
            ...
            if do_degrees is not None:
                ...
                if dataset == 'h36m':
                    cv2.imwrite(out_path.replace('.mat', '.png'), vis[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='lsp',
        type=str,
        help='Currently supports lsp (default) and h36m.')
    parser.add_argument(
        '--n_batches',
        default=1,
        type=int,
    ...
    main(args.base_dir, args.out_dir, args.dataset, args.n_batches, args.batch_id,
         use_interpenetration, args.n_betas, args.flength, args.side_view_thsh,
         args.gender_neutral, args.viz)
